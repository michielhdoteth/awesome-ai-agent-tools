...
function getEmoji(type) {
  return { permission_prompt: 'LOCK', idle_prompt: 'WAIT', elicitation_dialog: 'TOOL' }[type] || 'BELL';
}
  ...
  const payload = {
    blocks: [
      ...
      },
      {
        type: 'context',
        elements: [
          { type: 'mrkdwn', text: `Folder: \`${data.cwd || 'unknown'}\`` },
          { type: 'mrkdwn', text: `Time: ${new Date().toLocaleTimeString()}` },
        ],
      },
    ],
  };
